...
select 日期 as cdate, organization_code as saletype,sum(生产总点数) as pointcount from CUX_SMT_PROD_RP 
where 工单类型 = '正常' and (装配件描述  not like '%F1专用%' and   装配件描述 not like '%F1 TB%') and to_char(日期,'yyyy-mm')='2017-05'
 group by 日期, organization_code 
 order by 日期
 
 select * from CUX_SMT_PROD_RP 
where 工单类型 = '正常' and (装配件描述  not like '%F1专用%' and   装配件描述 not like '%F1 TB%') and to_char(日期,'yyyy-mm-dd')='2017-05-02'

select * from cux_wip_smt_doneinfo CS where to_char(created_date,'yyyy-mm-dd')='2017-05-02'


select * from (
 select 日期 as cdate,工单号,count(*) as fnum from CUX_SMT_PROD_RP 
where 工单类型 = '正常' and (装配件描述  not like '%F1专用%' and   装配件描述 not like '%F1 TB%') and to_char(日期,'yyyy-mm-dd')='2017-05-02' group by 日期,工单号 order by 日期) where fnum >1
