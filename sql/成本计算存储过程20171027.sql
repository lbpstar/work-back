...
select top (@days) @cmonth + '-' + cdate as cdate,saletypeid into #temp_days from #temp
 ...
 --单日成本
  ...
  begin
	  ...
	  cast((isnull(d.cost,0) + isnull(i.cost,0) + isnull(dp.DEPRECIATION,0) + isnull(re.RENT_EXPENSE,0)*2 + isnull(we.WATER_ELECTRICITY,0))/CAST(r.cost_rate as decimal(18,4)) as decimal(18,5)) as 预估成本,
	  预估单点成本= case when s.POINTCOUNT is null or s.POINTCOUNT = 0 then 0 else cast(((isnull(d.cost,0) + isnull(i.cost,0) + isnull(dp.DEPRECIATION,0) + isnull(re.RENT_EXPENSE,0)*2 + isnull(we.WATER_ELECTRICITY,0))/CAST(r.cost_rate as decimal(18,4)))/s.POINTCOUNT as decimal(18,5)) end,
  ...
  begin
	  ...
	  select st.cid saletypeid,st.cname 营业类型,t.cdate 日期,d.chours 直接人力小时数,d.cost 直接人力成本,i.chours 间接人力小时数,i.cost 间接人力成本,dp.DEPRECIATION 折旧费,re.RENT_EXPENSE*2 租赁费,
	  we.WATER_ELECTRICITY 水电费,s.POINTCOUNT 生产点数,
	  cast((isnull(d.cost,0) + isnull(i.cost,0) + isnull(dp.DEPRECIATION,0) + isnull(re.RENT_EXPENSE,0)*2 + isnull(we.WATER_ELECTRICITY,0))/CAST(r.cost_rate as decimal(18,4))*(1-n.STEEL_NET_RATE) as decimal(18,5)) as 预估成本,
	  预估单点成本= case when s.POINTCOUNT is null or s.POINTCOUNT = 0 then 0 else cast(((isnull(d.cost,0) + isnull(i.cost,0) + isnull(dp.DEPRECIATION,0) + isnull(re.RENT_EXPENSE,0)*2 + isnull(we.WATER_ELECTRICITY,0))/CAST(r.cost_rate as decimal(18,4))*(1-n.STEEL_NET_RATE) - isnull(re.RENT_EXPENSE,0))/s.POINTCOUNT as decimal(18,5)) end,
...
--月度成本
 --营业类型、月份、直接人力小时数、直接人力成本、间接人力小时数、间接人力成本、折旧费、租赁费、水电费、生产点数、预估成本、预估单点成本、标准单点成本、标准成本、盈亏
  ...
  begin
	  ...
	  cast((isnull(t.cost,0) + isnull(t.incost,0) + isnull(dp.DEPRECIATION,0) + isnull(re.RENT_EXPENSE,0)*2 + isnull(we.WATER_ELECTRICITY,0))/CAST(r.cost_rate as decimal(18,4)) as decimal(18,5)) as 预估成本,
	  预估单点成本= case when t.POINTCOUNT is null or t.POINTCOUNT = 0 then 0 else cast(((isnull(t.cost,0) + isnull(t.incost,0) + isnull(dp.DEPRECIATION,0) + isnull(re.RENT_EXPENSE,0)*2 + isnull(we.WATER_ELECTRICITY,0))/CAST(r.cost_rate as decimal(18,4)))/t.POINTCOUNT as decimal(18,5)) end,
  ...
  begin
	  ...
	  select st.cid saletypeid,st.cname 营业类型,@cmonth 月份,t.chours 直接人力小时数,t.cost 直接人力成本,t.inhours 间接人力小时数,t.incost 间接人力成本,dp.DEPRECIATION 折旧费,re.RENT_EXPENSE*2 租赁费,
	  we.WATER_ELECTRICITY 水电费,t.POINTCOUNT 生产点数,
	  cast((isnull(t.cost,0) + isnull(t.incost,0) + isnull(dp.DEPRECIATION,0) + isnull(re.RENT_EXPENSE,0)*2 + isnull(we.WATER_ELECTRICITY,0))/CAST(r.cost_rate as decimal(18,4))*(1-n.STEEL_NET_RATE) as decimal(18,5)) as 预估成本,
	  预估单点成本= case when t.POINTCOUNT is null or t.POINTCOUNT = 0 then 0 else cast(((isnull(t.cost,0) + isnull(t.incost,0) + isnull(dp.DEPRECIATION,0) + isnull(re.RENT_EXPENSE,0)*2 + isnull(we.WATER_ELECTRICITY,0))/CAST(r.cost_rate as decimal(18,4))*(1-n.STEEL_NET_RATE) - isnull(re.RENT_EXPENSE,0))/t.POINTCOUNT as decimal(18,5)) end,
	  p.STANDARD_POINT as 标准单点成本,
