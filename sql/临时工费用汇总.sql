alter procedure [dbo].[COST_TEMP_EMPLOYEE_SALARY_SUM]
@cmonth varchar(20),@dept1 varchar(50),@dept2 varchar(200),@dept3 varchar(200)
as
...
if @dept3 <>'0'
begin 
select SUM(本月工时数) 本月工时数,SUM(应结算工时数) 应结算工时数, sum(餐补) 餐补,SUM(夜班天数) 夜班天数,SUM(夜班补助) 夜班补助,SUM(车补) 车补,sum(交通补贴) 交通补贴,SUM(转正费) 转正费,SUM(应付工资) 应付工资,SUM(转正费) + SUM(应付工资) as 总费用,SUM(工作日时数) 工作日时数,SUM(工作日8H内时数) 工作日8H内时数,SUM(工作日8H外时数) 工作日8H外时数,SUM(周末时数) 周末时数,SUM(节假日时数) 节假日时数,SUM(节省费用) 节省费用,sum(上月未结算工时) 上月未结算工时,SUM(上月未结算餐补) 上月未结算餐补,SUM(上月未结算夜班补助) 上月未结算夜班补助,sum(上月未结算交通补贴) 上月未结算交通补贴,SUM(本月未结算工时) 本月未结算工时 into #temp10 from #temp9 where  三级部门 = @dept3 and 二级部门 = @dept2 and 一级部门 = @dept1
select * from #temp10
end
else if @dept2 <>'0'
begin 
select SUM(本月工时数) 本月工时数,SUM(应结算工时数) 应结算工时数, sum(餐补) 餐补,SUM(夜班天数) 夜班天数,SUM(夜班补助) 夜班补助,SUM(车补) 车补,sum(交通补贴) 交通补贴,SUM(转正费) 转正费,SUM(应付工资) 应付工资,SUM(转正费) + SUM(应付工资) as 总费用,SUM(工作日时数) 工作日时数,SUM(工作日8H内时数) 工作日8H内时数,SUM(工作日8H外时数) 工作日8H外时数,SUM(周末时数) 周末时数,SUM(节假日时数) 节假日时数,SUM(节省费用) 节省费用,sum(上月未结算工时) 上月未结算工时,SUM(上月未结算餐补) 上月未结算餐补,SUM(上月未结算夜班补助) 上月未结算夜班补助,sum(上月未结算交通补贴) 上月未结算交通补贴,SUM(本月未结算工时) 本月未结算工时 into #temp11 from #temp9 where  二级部门 = @dept2 and 一级部门 = @dept1
select * from #temp11
end
else if @dept1 <>'0'
begin 
select 本月工时数,应结算工时数, 餐补,夜班天数,夜班补助,车补,交通补贴,转正费,e.老师补贴,应付工资,转正费+ isnull(e.老师补贴,0)+ 应付工资 as 总费用,工作日时数,工作日8H内时数,工作日8H外时数,周末时数,节假日时数,节省费用-isnull(e.老师补贴,0) as 节省费用,上月未结算工时,上月未结算餐补,上月未结算夜班补助,上月未结算交通补贴,本月未结算工时  into #temp12 from 
(select SUM(本月工时数) 本月工时数,SUM(应结算工时数) 应结算工时数, sum(餐补) 餐补,SUM(夜班天数) 夜班天数,SUM(夜班补助) 夜班补助,SUM(车补) 车补,sum(交通补贴) 交通补贴,SUM(转正费) 转正费,SUM(应付工资) 应付工资,SUM(工作日时数) 工作日时数,SUM(工作日8H内时数) 工作日8H内时数,SUM(工作日8H外时数) 工作日8H外时数,SUM(周末时数) 周末时数,SUM(节假日时数) 节假日时数,SUM(节省费用) 节省费用,SUM(上月未结算天数) 上月未结算天数,sum(上月未结算工时) 上月未结算工时,SUM(上月未结算餐补) 上月未结算餐补,SUM(上月未结算夜班补助) 上月未结算夜班补助,sum(上月未结算交通补贴) 上月未结算交通补贴,SUM(本月未结算工时) 本月未结算工时 
 from #temp9 
where  一级部门 = @dept1) t
left join (select SUM(EXPENSE) 老师补贴 from COST_EXPENSE where CTYPE = 1 and cmonth = @cmonth ) e on 1 = 1
select * from #temp12
end
else
begin 
select 本月工时数,应结算工时数, 餐补,夜班天数,夜班补助,车补,交通补贴,转正费,e.老师补贴,应付工资,转正费+ isnull(e.老师补贴,0)+ 应付工资 as 总费用,工作日时数,工作日8H内时数,工作日8H外时数,周末时数,节假日时数,节省费用-isnull(e.老师补贴,0) as 节省费用,上月未结算工时,上月未结算餐补,上月未结算夜班补助,上月未结算交通补贴,本月未结算工时  into #temp13 from 
(select SUM(本月工时数) 本月工时数,SUM(应结算工时数) 应结算工时数, sum(餐补) 餐补,SUM(夜班天数) 夜班天数,SUM(夜班补助) 夜班补助,SUM(车补) 车补,sum(交通补贴) 交通补贴,SUM(转正费) 转正费,SUM(应付工资) 应付工资,SUM(工作日时数) 工作日时数,SUM(工作日8H内时数) 工作日8H内时数,SUM(工作日8H外时数) 工作日8H外时数,SUM(周末时数) 周末时数,SUM(节假日时数) 节假日时数,SUM(节省费用) 节省费用,SUM(上月未结算天数) 上月未结算天数,sum(上月未结算工时) 上月未结算工时,SUM(上月未结算餐补) 上月未结算餐补,SUM(上月未结算夜班补助) 上月未结算夜班补助,sum(上月未结算交通补贴) 上月未结算交通补贴,SUM(本月未结算工时) 本月未结算工时 
 from #temp9 ) t
left join (select SUM(EXPENSE) 老师补贴 from COST_EXPENSE where CTYPE = 1 and cmonth = @cmonth ) e on 1 = 1
select * from #temp13
