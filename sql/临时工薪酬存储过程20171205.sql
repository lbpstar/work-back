...
create table #temp(cno nvarchar(20),cname nvarchar(20),SEX nvarchar(20),REGISTER_DATE date,LEAVE_DATE date,CFROM nvarchar(200),FROM_TYPE INT,dept2 nvarchar(200),dept3 nvarchar(200),STATUS nvarchar(50),
begin_date date,end_date date,shift nvarchar(50),HOURS decimal(18,2),NORMAL_HOURS decimal(18,2),OVERTIME_HOURS decimal(18,2),PRICE decimal(18,2),MEAL_PRICE decimal(18,2),NIGHT_PRICE decimal(18,2),TRAVEL_PRICE decimal(18,2),REGULAR_PRICE decimal(18,2),standard_price decimal(18,2),insurance_price decimal(18,2),salary decimal(18,4),salary2 decimal(18,4),work_type int)
--每人每天的工资情况；和出勤价格时间范围匹配时，以报道日期为准(后期可能调整为按出勤日期)
if @cno <> ''
begin
insert into #temp 
select e.CNO ,e.CNAME ,e.SEX ,e.REGISTER_DATE ,e.LEAVE_DATE ,e.CFROM ,e.from_type,e.DEPT2 ,e.DEPT3 ,e.STATUS,a.BEGIN_DATE ,a.END_DATE ,shift = case when isnull(a.shift,'') <> '' then a.shift else e.shift end ,isnull(a.HOURS,0) HOURS,isnull(a.NORMAL_HOURS,0) NORMAL_HOURS ,isnull(a.OVERTIME_HOURS,0) OVERTIME_HOURS,
isnull(p.PRICE,0) PRICE, isnull(p.MEAL_PRICE,0) MEAL_PRICE,isnull(p.NIGHT_PRICE,0) NIGHT_PRICE,isnull(p.TRAVEL_PRICE,0) TRAVEL_PRICE,isnull(p.regular_price,0) regular_price,isnull(p2.price,0) standard_price,isnull(p2.insurance_price,0) insurance_price,isnull(a.HOURS,0)* isnull(p.PRICE,0) salary,
...
--每人合计
select cno,max(p.CNAME) CNAME,max(SEX) SEX,max(REGISTER_DATE) REGISTER_DATE,max(LEAVE_DATE) LEAVE_DATE,max(CFROM) CFROM,max(d.cname) from_type,max(DEPT2) DEPT2,max(DEPT3) DEPT3,max(STATUS) STATUS,COUNT(*) sum_days,
sum(isnull(HOURS,0)) SUM_HOURS,sum(isnull(NORMAL_HOURS,0)) NORMAL_HOURS,sum(isnull(OVERTIME_HOURS,0)) OVERTIME_HOURS,SUM(salary) salary,SUM(salary2) salary2,SUM(MEAL_PRICE) MEAL,MAX(TRAVEL_PRICE) travel,case when max(STATUS) = '转正' and max(LEAVE_DATE) >= @begin and max(LEAVE_DATE) <= @end then MAX(REGULAR_PRICE) else 0 end regular,min(insurance_price) insurance  
...
sum(p.MEAL_PRICE) meal,SUM(case when case when isnull(a.shift,'') <> '' then a.shift else e.shift end like '夜班%' then p.NIGHT_PRICE else 0 end) NIGHT into #temp6 
...
--工时汇总
select t1.CNO 工号,t1.CNAME 姓名,t1.SEX 性别,t1.REGISTER_DATE 报道日期,t1.LEAVE_DATE 离职日期,t1.CFROM 输送渠道,t1.from_type 输送类型,t1.DEPT2 二级部门,t1.DEPT3 三级部门,t1.STATUS 状态,t1.sum_days 在岗天数,
...
case when (t1.REGISTER_DATE >= @begin and t1.REGISTER_DATE <= @end and t8.sum_hours24 IS null) OR (t6.last_hours24 is not null and t8.sum_hours24 IS null) then t1.travel else 0 end 车补, t1.regular 转正费,
...
case when t8.sum_hours24 IS null then t1.salary2 + t1.insurance + ISNULL(t6.last_hours24_salary2,0) - (t1.salary+t1.MEAL + isnull(t5.salary4,0)+ ISNULL(t6.last_hours24_salary,0) + ISNULL(t6.meal,0)+ ISNULL(t6.night,0) + case when (t1.REGISTER_DATE >= @begin and t1.REGISTER_DATE <= @end and t8.sum_hours24 IS null) OR (t6.last_hours24 is not null and t8.sum_hours24 IS null) then t1.travel else 0 end)  else 0 end 节省费用,
