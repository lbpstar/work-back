...
namespace SMTCost
{
    public partial class ProductCostDayPointChart : DevExpress.XtraEditors.XtraForm
    {
        public ProductCostDayPointChart()
        {
            InitializeComponent();
        }
        private static ProductCostDayPointChart smtcdcform = null;

        public static ProductCostDayPointChart GetInstance()
        {
            if (smtcdcform == null || smtcdcform.IsDisposed)
            {
                smtcdcform = new ProductCostDayPointChart();
            }
            return smtcdcform;
        }
        ...
        private void BindChart()
        {
            ...
            //预估成本
            Series series1 = new Series("预估单台成本", DevExpress.XtraCharts.ViewType.Spline);
            series1.ArgumentDataMember = dt.Columns["日期"].ToString();
            series1.ValueDataMembersSerializable = dt.Columns["预估单台成本"].ToString();
            chartControlDay.Series.Add(series1);

            //标准成本
            Series series2 = new Series("标准单台成本", DevExpress.XtraCharts.ViewType.Spline);
            series2.ArgumentDataMember = dt.Columns["日期"].ToString();
            series2.ValueDataMembersSerializable = dt.Columns["标准单台成本"].ToString();
