...
namespace SMTCost
{
    public partial class EMSSMTCostDeptDay : DevExpress.XtraEditors.XtraForm
    {
        public EMSSMTCostDeptDay()
        {
            InitializeComponent();
        }
        private static EMSSMTCostDeptDay smtddform = null;

        public static EMSSMTCostDeptDay GetInstance()
        {
            if (smtddform == null || smtddform.IsDisposed)
            {
                smtddform = new EMSSMTCostDeptDay();
            }
            return smtddform;
        }
        ...
        private void ShowDetail()
        {
            ...
            if(deptid == 0)
            {
                ...
                strsql = strsql + " union select SALE_TYPE_NAME 营业类型,dept_name 部门,'汇总：' 日期,DIRECT_HOURS 直接人工小时数,cast(round(DIRECT_COST, 2) as decimal(18, 2)) 直接人工成本,INDIRECT_HOURS 间接人工小时数,cast(round(INDIRECT_COST, 2) as decimal(18, 2)) 间接人工成本,cast(round(POINTCOUNT, 2) as decimal(18, 2)) 点数,cast(round(COST, 2) as decimal(18, 2)) 预估成本,COST_POINT 预估单点成本,STANDARD_POINT 标准单点成本,cast(round(STANDARD_COST, 2) as decimal(18, 2)) 标准成本,cast(round(PROFIT, 2) as decimal(18, 2)) 盈亏 from COST_DEPT_MONTH_CALCULATE where cmonth = '" + month + "' and sale_type_id = 10) i order by 部门";
            }
            else
            {
                strsql = "select * from (select SALE_TYPE_NAME 营业类型,dept_name 部门,CONVERT(varchar(100), CDATE, 23) 日期,DIRECT_HOURS 直接人工小时数,cast(round(DIRECT_COST, 2) as decimal(18, 2)) 直接人工成本,INDIRECT_HOURS 间接人工小时数,cast(round(INDIRECT_COST, 2) as decimal(18, 2)) 间接人工成本,cast(round(POINTCOUNT, 2) as decimal(18, 2)) 点数,cast(round(COST, 2) as decimal(18, 2)) 预估成本,COST_POINT 预估单点成本,STANDARD_POINT 标准单点成本,cast(round(STANDARD_COST, 2) as decimal(18, 2)) 标准成本,cast(round(PROFIT, 2) as decimal(18, 2)) 盈亏 from COST_DEPT_CALCULATE where cdate like '" + month + "%' and sale_type_id = 10 and dept_id = " + deptid;
                strsql = strsql + " union select SALE_TYPE_NAME 营业类型,dept_name 部门,'汇总：' 日期,DIRECT_HOURS 直接人工小时数,cast(round(DIRECT_COST, 2) as decimal(18, 2)) 直接人工成本,INDIRECT_HOURS 间接人工小时数,cast(round(INDIRECT_COST, 2) as decimal(18, 2)) 间接人工成本,cast(round(POINTCOUNT, 2) as decimal(18, 2)) 点数,cast(round(COST, 2) as decimal(18, 2)) 预估成本,COST_POINT 预估单点成本,STANDARD_POINT 标准单点成本,cast(round(STANDARD_COST, 2) as decimal(18, 2)) 标准成本,cast(round(PROFIT, 2) as decimal(18, 2)) 盈亏 from COST_DEPT_MONTH_CALCULATE where cmonth = '" + month + "' and sale_type_id = 10 and dept_id = " + deptid + ") i order by 部门";
            }
