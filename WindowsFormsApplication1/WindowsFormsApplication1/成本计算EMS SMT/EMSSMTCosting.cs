...
namespace SMTCost
{
    public partial class EMSSMTCosting : DevExpress.XtraEditors.XtraForm
    {
        public EMSSMTCosting()
        {
            InitializeComponent();
        }
        private static EMSSMTCosting smtcform = null;

        public static EMSSMTCosting GetInstance()
        {
            if (smtcform == null || smtcform.IsDisposed)
            {
                smtcform = new EMSSMTCosting();
            }
            return smtcform;
        }
        ...
        private void simpleButton清空_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.OK)
            {
                ...
                sql1 = "delete from COST_DAY_CALCULATE where cdate like '" + dateTimePicker1.Text.ToString() + "%' and sale_type_id = 10";
                sql2 = "delete from COST_MONTH_CALCULATE where cmonth ='" + dateTimePicker1.Text.ToString() + "' and sale_type_id = 10";
                sql3 = "delete from COST_DEPT_CALCULATE where cdate like '" + dateTimePicker1.Text.ToString() + "%' and sale_type_id = 10";
                sql4 = "delete from COST_DEPT_MONTH_CALCULATE where cmonth ='" + dateTimePicker1.Text.ToString() + "' and sale_type_id = 10";
        ...
        private bool CheckData(bool ck)
        {
            ...
            sql = "select top 1 * from cost_pointcount_sum where saletype_id = 10 and cdate like '" + dateTimePicker1.Text.ToString() + "%'";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
            ...
            sql = "select top 1 * from cost_depreciation where YYYYMM = '" + dateTimePicker1.Text.ToString() + "' and SALE_TYPE_ID = 10";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
            ...
            sql = "select top 1 * from COST_RENT_EXPENSE where YYYYMM = '" + dateTimePicker1.Text.ToString() + "' and SALE_TYPE_ID = 10";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
            ...
            sql = "select top 1 * from COST_WATER_ELECTRICITY where YYYYMM = '" + dateTimePicker1.Text.ToString() + "' and SALE_TYPE_ID = 10";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
            ...
            sql = "select top 1 * from COST_STANDARD_POINT where YYYYMM = '" + dateTimePicker1.Text.ToString() + "' and SALE_TYPE_ID = 10";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
            ...
            sql = "select top 1 * from COST_DEPT_STANDARD_POINT where YYYYMM = '" + dateTimePicker1.Text.ToString() + "' and SALE_TYPE_ID = 10";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
            ...
            sql = "select top 1 * from COST_STEEL_NET_RATE where YYYYMM = '" + dateTimePicker1.Text.ToString() + "'";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
                MessageBox.Show("没有钢网成本占比！");
                return false;
            }
            ...
            sql = "select top 1 * from cost_rate where yyyy ='" + yyyy + "' and isnull(dept_id,0) >0 and quarter_id = " + quarter + " and sale_type_id = 10";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
