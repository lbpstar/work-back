...
namespace SMTCost
{
    public partial class EMSSMTCostDeptDayPointChart : DevExpress.XtraEditors.XtraForm
    {
        public EMSSMTCostDeptDayPointChart()
        {
            InitializeComponent();
        }
        private static EMSSMTCostDeptDayPointChart smtcddcform = null;

        public static EMSSMTCostDeptDayPointChart GetInstance()
        {
            if (smtcddcform == null || smtcddcform.IsDisposed)
            {
                smtcddcform = new EMSSMTCostDeptDayPointChart();
            }
            return smtcddcform;
        }
        ...
        private void BindChart()
        {
            ...
            if(deptid==0)
            {
                strsql = "select SALE_TYPE_NAME 营业类型,dept_name 部门,Cdate 日期,DIRECT_HOURS 直接人工小时数,cast(round(DIRECT_COST, 2) as decimal(18, 2)) 直接人工成本,INDIRECT_HOURS 间接人工小时数,cast(round(INDIRECT_COST, 2) as decimal(18, 2)) 间接人工成本,cast(round(POINTCOUNT, 2) as decimal(18, 2)) 点数,cast(round(COST, 2) as decimal(18, 2)) 预估成本,COST_POINT 预估单点成本,STANDARD_POINT 标准单点成本,cast(round(isnull(STANDARD_COST,0), 2) as decimal(18, 2)) 标准成本,cast(round(PROFIT, 2) as decimal(18, 2)) 盈亏 from COST_DEPT_CALCULATE where cdate like '" + month + "%' and sale_type_id = 10";
                strsql2 = "select distinct dept_name 部门 from COST_DEPT_CALCULATE where cdate like '" + month + "%' and sale_type_id = 10";
            }
            else
            {
                strsql = "select SALE_TYPE_NAME 营业类型,dept_name 部门,Cdate 日期,DIRECT_HOURS 直接人工小时数,cast(round(DIRECT_COST, 2) as decimal(18, 2)) 直接人工成本,INDIRECT_HOURS 间接人工小时数,cast(round(INDIRECT_COST, 2) as decimal(18, 2)) 间接人工成本,cast(round(POINTCOUNT, 2) as decimal(18, 2)) 点数,cast(round(COST, 2) as decimal(18, 2)) 预估成本,COST_POINT 预估单点成本,STANDARD_POINT 标准单点成本,cast(round(isnull(STANDARD_COST,0), 2) as decimal(18, 2)) 标准成本,cast(round(PROFIT, 2) as decimal(18, 2)) 盈亏 from COST_DEPT_CALCULATE where cdate like '" + month + "%' and sale_type_id = 10 and dept_id =" + deptid;
                strsql2 = "select distinct dept_name 部门 from COST_DEPT_CALCULATE where cdate like '" + month + "%' and sale_type_id = 10 and dept_id = " + deptid;
            }
        ...
        {
            ConnDB conn = new ConnDB();
            string sql = "select d.cid,d.cname from  COST_DEPT d where d.saletype_id = 10";
