...
namespace SMTCost
{
    public partial class SMTHHCostDeptDayPointChart : DevExpress.XtraEditors.XtraForm
    {
        public SMTHHCostDeptDayPointChart()
        {
            InitializeComponent();
        }
        private static SMTHHCostDeptDayPointChart smtcddcform = null;

        public static SMTHHCostDeptDayPointChart GetInstance()
        {
            if (smtcddcform == null || smtcddcform.IsDisposed)
            {
                smtcddcform = new SMTHHCostDeptDayPointChart();
            }
            return smtcddcform;
        }
        ...
        private void BindChart()
        {
            ...
            {
                //预估成本
                myseries.Add(new Series(ds2.Tables[0].Rows[i][0].ToString() + "预估单小时成本", DevExpress.XtraCharts.ViewType.Spline));
                myseries[i * 2].ArgumentDataMember = dt.Columns["日期"].ToString();
                myseries[i * 2].ValueDataMembersSerializable = dt.Columns["预估单小时成本"].ToString();
                chartControlDeptDay.Series.Add(myseries[i * 2]);
                //标准成本
                myseries.Add(new Series(ds2.Tables[0].Rows[i][0].ToString() + "标准单小时成本", DevExpress.XtraCharts.ViewType.Spline));
                myseries[i * 2 + 1].ArgumentDataMember = dt.Columns["日期"].ToString();
                myseries[i * 2 + 1].ValueDataMembersSerializable = dt.Columns["标准单小时成本"].ToString();
        ...
        private void SMTHHCostDeptDayChart_Load(object sender, EventArgs e)
        {
            SetTitle();
            BindDept();
            BindChart();
