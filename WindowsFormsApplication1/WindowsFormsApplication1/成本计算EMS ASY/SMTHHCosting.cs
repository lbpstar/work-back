...
namespace SMTCost
{
    public partial class SMTHHCosting : DevExpress.XtraEditors.XtraForm
    {
        public SMTHHCosting()
        {
            InitializeComponent();
        }
        private static SMTHHCosting smtcform = null;

        public static SMTHHCosting GetInstance()
        {
            if (smtcform == null || smtcform.IsDisposed)
            {
                smtcform = new SMTHHCosting();
            }
            return smtcform;
        }
        ...
        private void simpleButton成本计算_Click(object sender, EventArgs e)
        {
            ...
            if(isok)
            {
                sql = "select * from COST_MONTH_CALCULATE where cmonth ='" + dateTimePicker1.Text.ToString() + "' and sale_type_id =13";
        ...
        private void simpleButton清空_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.OK)
            {
                ...
                sql1 = "delete from COST_DAY_CALCULATE where cdate like '" + dateTimePicker1.Text.ToString() + "%' and sale_type_id =13";
                sql2 = "delete from COST_MONTH_CALCULATE where cmonth ='" + dateTimePicker1.Text.ToString() + "' and sale_type_id =13";
                sql3 = "delete from COST_DEPT_CALCULATE where cdate like '" + dateTimePicker1.Text.ToString() + "%' and sale_type_id =13";
                sql4 = "delete from COST_DEPT_MONTH_CALCULATE where cmonth ='" + dateTimePicker1.Text.ToString() + "' and sale_type_id =13";
        ...
        private bool CheckData(bool ck)
        {
            ...
            sql = "select top 1 * from COST_EMS_HH_HOURS where cdate like '" + dateTimePicker1.Text.ToString() + "%'";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
                MessageBox.Show("没有产出工时数据！");
                return false;
            }
            sql = "select top 1 * from COST_STANDARD_POINT where YYYYMM = '" + dateTimePicker1.Text.ToString() + "' and SALE_TYPE_ID = 13";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
                MessageBox.Show("没有标准单小时成本数据！");
                return false;
            }
            sql = "select top 1 * from COST_DEPT_STANDARD_POINT where YYYYMM = '" + dateTimePicker1.Text.ToString() + "' and SALE_TYPE_ID = 13";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
                MessageBox.Show("没有部门标准单小时成本数据！");
                return false;
            }
            ...
            sql = "select top 1 * from cost_rate where yyyy ='" + yyyy + "' and isnull(dept_id,0) >0 and quarter_id = " + quarter + " and sale_type_id = 13";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
        ...
        private void SMTHHCosting_Load(object sender, EventArgs e)
        {
