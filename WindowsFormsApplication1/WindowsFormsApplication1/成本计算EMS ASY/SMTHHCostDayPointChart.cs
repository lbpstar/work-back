...
namespace SMTCost
{
    public partial class SMTHHCostDayPointChart : DevExpress.XtraEditors.XtraForm
    {
        public SMTHHCostDayPointChart()
        {
            InitializeComponent();
        }
        private static SMTHHCostDayPointChart smtcdcform = null;

        public static SMTHHCostDayPointChart GetInstance()
        {
            if (smtcdcform == null || smtcdcform.IsDisposed)
            {
                smtcdcform = new SMTHHCostDayPointChart();
            }
            return smtcdcform;
        }
        ...
        private void BindChart()
        {
            ...
            //预估成本
            Series series1 = new Series("预估单小时成本", DevExpress.XtraCharts.ViewType.Spline);
            series1.ArgumentDataMember = dt.Columns["日期"].ToString();
            series1.ValueDataMembersSerializable = dt.Columns["预估单小时成本"].ToString();
            chartControlDay.Series.Add(series1);

            //标准成本
            Series series2 = new Series("标准单小时成本", DevExpress.XtraCharts.ViewType.Spline);
            series2.ArgumentDataMember = dt.Columns["日期"].ToString();
            series2.ValueDataMembersSerializable = dt.Columns["标准单小时成本"].ToString();
