...
namespace SMTCost
{
    public partial class SystemCostDeptDayPointChart : DevExpress.XtraEditors.XtraForm
    {
        public SystemCostDeptDayPointChart()
        {
            InitializeComponent();
        }
        private static SystemCostDeptDayPointChart smtcddcform = null;

        public static SystemCostDeptDayPointChart GetInstance()
        {
            if (smtcddcform == null || smtcddcform.IsDisposed)
            {
                smtcddcform = new SystemCostDeptDayPointChart();
            }
            return smtcddcform;
        }
        ...
        private void BindChart()
        {
            ...
            if(deptid==0)
            {
                strsql = "select SALE_TYPE_NAME 营业类型,dept_name 部门,Cdate 日期,DIRECT_HOURS 直接人工小时数,cast(round(DIRECT_COST, 2) as decimal(18, 2)) 直接人工成本,INDIRECT_HOURS 间接人工小时数,cast(round(INDIRECT_COST, 2) as decimal(18, 2)) 间接人工成本,cast(round(POINTCOUNT, 2) as decimal(18, 2)) 产出信道数,cast(round(COST, 2) as decimal(18, 2)) 预估成本,COST_POINT 预估单信道成本,STANDARD_POINT 标准单信道成本,cast(round(isnull(STANDARD_COST,0), 2) as decimal(18, 2)) 标准成本,cast(round(PROFIT, 2) as decimal(18, 2)) 盈亏 from COST_DEPT_CALCULATE where cdate like '" + month + "%' and sale_type_id =15";
                strsql2 = "select distinct dept_name 部门 from COST_DEPT_CALCULATE where cdate like '" + month + "%' and sale_type_id =15";
            }
            else
            {
                strsql = "select SALE_TYPE_NAME 营业类型,dept_name 部门,Cdate 日期,DIRECT_HOURS 直接人工小时数,cast(round(DIRECT_COST, 2) as decimal(18, 2)) 直接人工成本,INDIRECT_HOURS 间接人工小时数,cast(round(INDIRECT_COST, 2) as decimal(18, 2)) 间接人工成本,cast(round(POINTCOUNT, 2) as decimal(18, 2)) 产出信道数,cast(round(COST, 2) as decimal(18, 2)) 预估成本,COST_POINT 预估单信道成本,STANDARD_POINT 标准单信道成本,cast(round(isnull(STANDARD_COST,0), 2) as decimal(18, 2)) 标准成本,cast(round(PROFIT, 2) as decimal(18, 2)) 盈亏 from COST_DEPT_CALCULATE where cdate like '" + month + "%' and sale_type_id =15 and dept_id =" + deptid;
                strsql2 = "select distinct dept_name 部门 from COST_DEPT_CALCULATE where cdate like '" + month + "%' and sale_type_id =15 and dept_id = " + deptid;
            }
            ...
            {
                //预估成本
                myseries.Add(new Series(ds2.Tables[0].Rows[i][0].ToString() + "预估单信道成本", DevExpress.XtraCharts.ViewType.Spline));
                myseries[i * 2].ArgumentDataMember = dt.Columns["日期"].ToString();
                myseries[i * 2].ValueDataMembersSerializable = dt.Columns["预估单信道成本"].ToString();
                chartControlDeptDay.Series.Add(myseries[i * 2]);
                //标准成本
                myseries.Add(new Series(ds2.Tables[0].Rows[i][0].ToString() + "标准单信道成本", DevExpress.XtraCharts.ViewType.Spline));
                myseries[i * 2 + 1].ArgumentDataMember = dt.Columns["日期"].ToString();
                myseries[i * 2 + 1].ValueDataMembersSerializable = dt.Columns["标准单信道成本"].ToString();
        ...
        {
            ConnDB conn = new ConnDB();
            string sql = "select d.cid,d.cname from  COST_DEPT d where d.saletype_id = 15";
        ...
        private void SystemCostDeptDayChart_Load(object sender, EventArgs e)
        {
            SetTitle();
            BindDept();
            BindChart();
