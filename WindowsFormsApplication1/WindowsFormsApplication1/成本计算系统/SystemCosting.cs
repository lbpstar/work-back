...
namespace SMTCost
{
    public partial class SystemCosting : DevExpress.XtraEditors.XtraForm
    {
        public SystemCosting()
        {
            InitializeComponent();
        }
        private static SystemCosting smtcform = null;

        public static SystemCosting GetInstance()
        {
            if (smtcform == null || smtcform.IsDisposed)
            {
                smtcform = new SystemCosting();
            }
            return smtcform;
        }
        ...
        private void simpleButton成本计算_Click(object sender, EventArgs e)
        {
            ...
            if(isok)
            {
                sql = "select * from COST_MONTH_CALCULATE where cmonth ='" + dateTimePicker1.Text.ToString() + "' and sale_type_id =15";
        ...
        private bool CheckData(bool ck)
        {
            ...
            sql = "select top 1 * from COST_PRODUCT_QUANTITY where type = 2 and PRODUCT_QUANTITY >0 and cdate like '" + dateTimePicker1.Text.ToString() + "%'";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
                MessageBox.Show("没有信道数！");
                return false;
            }
            sql = "select top 1 * from cost_depreciation where YYYYMM = '" + dateTimePicker1.Text.ToString() + "' and SALE_TYPE_ID = 15";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
            ...
            sql = "select top 1 * from COST_STANDARD_POINT where YYYYMM = '" + dateTimePicker1.Text.ToString() + "' and SALE_TYPE_ID = 15";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
                MessageBox.Show("没有标准单信道成本数据！");
                return false;
            }
            sql = "select top 1 * from COST_DEPT_STANDARD_POINT where YYYYMM = '" + dateTimePicker1.Text.ToString() + "' and SALE_TYPE_ID = 15";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
                MessageBox.Show("没有部门标准单信道成本数据！");
                return false;
            }
            ...
            sql = "select top 1 * from cost_rate where yyyy ='" + yyyy + "' and isnull(dept_id,0) >0 and sale_type_id = 15 and quarter_id = " + quarter;
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
            ...
            sql = "select top 1 * from COST_RATE where yyyy ='" + yyyy + "' and QUARTER_ID = " + quarter + " and isnull(dept_id,0) = 0 and sale_type_id = 15";
            count = conn.ReturnRecordCount(sql);
            if (count == 0)
            {
        ...
        private void SystemCosting_Load(object sender, EventArgs e)
        {
